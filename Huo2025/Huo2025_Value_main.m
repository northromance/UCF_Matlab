function [Value_data,Rcost,cost_sum,net_profit, initial_coalition]= Huo2025_Value_main(agents,tasks,Graph)
% =========================================================================
%  函数名称：Value_main
%
%  算法主要功能：
...
Value_Params=Huo_Value_init(length(agents),length(tasks));
...
for counter=1:50
    ...
    while( doneflag==0)
        ...
        for ii=1:Value_Params.N
            [incremental(ii),curnumberrow(ii),Value_data(ii)]=Huo_Value_order(agents, tasks, Value_data(ii), Value_Params);
            incremental(ii);
        end
        ...
        Value_data=Huo_Value_communication(agents, tasks, Value_data, Value_Params,Graph);%邻居agent间彼此通信
